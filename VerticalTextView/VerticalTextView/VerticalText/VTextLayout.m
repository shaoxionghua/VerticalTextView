...
#import "VTextLine.h"
#import "VTextUtilities.h"

static CGColorRef VTextGetCGColor(CGColorRef color) {
...
- (instancetype)initWithText:(NSAttributedString *)text vertical:(BOOL)vertical size:(CGSize)size{
    ...
    if(self){
        ...
        _vertical = vertical;
        _textAlignment = VTextAlignmentLeft;
...
//    }
    ...
    for (NSUInteger i = 0; i < lineCount;  i ++) {
        ...
        VTextLine* foLine = [[VTextLine alloc] initWithCTLine:ctLine position:position vertical:self.vertical];
    ...
    if(self.vertical){
        ////// vertical text /////
        NSCharacterSet *rotateCharset = VTextVerticalRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = VTextVerticalRotateAndMoveCharacterSet();
        
        for (VTextLine * line in self.textLines) {
    ...
    VTextLine* line =[self.textLines lastObject];
    NSRange r = line.range;
    self.showedIndex = r.location+r.length;
...
- (void)handleVerticalLines:(VTextLine*)line rotateCharset:(NSCharacterSet*)rotateCharset rotateMoveCharset:(NSCharacterSet*)rotateMoveCharset{
    ...
    for (NSUInteger r = 0; r < runCount; r++) {
        ...
        BOOL isColorGlyph = VTextCTFontContainsColorBitmapGlyphs(font);
        
        NSUInteger prevIdx = 0;
        VTextRunGlyphDrawMode prevMode = VTextRunGlyphDrawModeHorizontal;
        ...
        for (NSUInteger g = 0; g < glyphCount; g++) {
            ...
            VTextRunGlyphDrawMode mode = glyphRotateMove ? VTextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? VTextRunGlyphDrawModeVerticalRotate : VTextRunGlyphDrawModeHorizontal);
            if (g == 0) {
                prevMode = mode;
            } else if (mode != prevMode) {
                VTextRunGlyphRange *aRange = [VTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
        ...
        if (prevIdx < glyphCount) {
            VTextRunGlyphRange *aRange = [VTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
            [runRanges addObject:aRange];
        }
    }
...
- (void)drawLine:(VTextLine*)line run:(CTRunRef)run context:(CGContextRef)context size:(CGSize)size range:(NSArray *)runRanges verticalOffset:(CGFloat)verticalOffset{
    ...
    if(self.textAlignment == VTextAlignmentRight){
    ...
    } else if(self.textAlignment == VTextAlignmentCenter){
    ...
    if (!self.vertical) { // draw run
        ...
        fillColor = VTextGetCGColor(fillColor);
        ...
        CGContextSaveGState(context);
        {
            ...
            for (VTextRunGlyphRange *oneRange in runRanges) {
                NSRange range = oneRange.glyphRangeInRun;
                NSUInteger rangeMax = range.location + range.length;
                VTextRunGlyphDrawMode mode = oneRange.drawMode;
                ...
                for (NSUInteger g = range.location; g < rangeMax; g++) {
                    CGContextSaveGState(context); {
                        ...
                        if (mode) { // CJK glyph, need rotated
                            ...
                            if (mode == VTextRunGlyphDrawModeVerticalRotateMove) {
                                x += w;
                                y += w;
                            }
                            CGContextSetTextPosition(context, x, y);
                        } else {
                            CGContextRotateCTM(context, VTextDegreesToRadians(-90));
                            CGContextSetTextPosition(context,
                                                     line.position.y - size.height + glyphPositions[g].x + alignmentOffset,
                                                     line.position.x + verticalOffset + glyphPositions[g].y);
                        }
                        
                        if (VTextCTFontContainsColorBitmapGlyphs(runFont)) {
